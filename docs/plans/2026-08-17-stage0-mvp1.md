...
- Produces: `get_execution_logger(log_file: Path) -> logging.Logger`
  ...
  confidence=0.94`. **Важно:** логгер получается через `logging.getLogger()`
  по фиксированному имени (singleton в рамках процесса), поэтому каждый
  вызов `get_execution_logger(log_file)` обязан переустанавливать хендлеры
  на переданный `log_file`, а не молча возвращать логгер с хендлерами от
  предыдущего вызова — иначе в тестовом сюите (Task 19/21/22 вызывают эту
  функцию с разными `tmp_path` в каждом тесте) более поздний вызов будет
  писать не в свой файл, а в файл, открытый самым первым вызовом за весь
  прогон pytest.
...
- Produces: `MainWindow(window_manager, screen_capture, input_recorder,
  ...
  templates_dir, screenshots_dir, start_emergency_stop: bool = True)` —
  QMainWindow с: комбобоксом окон (`_refresh_windows()` вызывает
  `window_manager.list_windows()`), списком правил (`_refresh_rules()`
  вызывает `rule_repository.list_all()`), кнопками Record/Pause/Stop/Play,
  полем ввода имени при сохранении Rule, текстовым виджетом лога.
  `MainWindow` сам создаёт `EmergencyStop(input_controller,
  on_trigger=self._playback_controller.abort, hotkey=config.
  EMERGENCY_STOP_KEY)` **после** создания `self._playback_controller` — это
  критично: F9 должен прерывать именно текущий `PlaybackEngine.run()`, а не
  no-op (раздел 10 спеки). `start_emergency_stop=False` (только для тестов)
  пропускает запуск реального глобального хука, оставляя объект доступным
  как `self._emergency_stop` для прямого вызова `._trigger()` в тестах.
...
`EmergencyStop` здесь больше не создаётся — этим теперь владеет `MainWindow`
(см. Step 3), чтобы F9 был привязан к реальному `PlaybackController.abort`,
а не к отдельному no-op колбэку.
