...
Статус (2026-08-21): все 5 пунктов пройдены на реальной Windows-машине.

Предусловие, выясненное при этой валидации: игра запускается от имени
администратора, поэтому приложение тоже должно быть запущено элевированным
(`run-as-admin.cmd`). Иначе Windows UIPI молча выбрасывает и синтезированный
ввод, и события низкоуровневого хука записи — при этом `SendInput` возвращает
успех, а в логе исправно появляются клики, которых на самом деле не было.
